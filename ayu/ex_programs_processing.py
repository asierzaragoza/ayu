...
def add_slr_to_tmbed(tmbed_file, out_file):
    ...
    tmbed_df.set_index('prot_ID', inplace=True)
    tmbed_df_col_values = tmbed_df.columns.values
    tmbed_closed = tmbed_df.apply(closure_func, axis=1)
    
    tmbed_closed.rename({k:v for k,v in zip(tmbed_closed.columns.values, 
                        tmbed_df_col_values) }, axis='columns', inplace=True)
    tmbed_closed.reset_index(inplace=True)
...
def signalp6_logit_transform(in_file, out_file):
    ...
    signalp_df['none_prob'][signalp_df['none_prob'] == 0] = epsilon
    signalp_df['sp_prob'][signalp_df['sp_prob'] >= 1] = 1-epsilon
    signalp_df['none_prob'][signalp_df['none_prob'] >= 1] = 1-epsilon
    
    signalp_df = signalp_df.apply(logit_func, axis=1)
    signalp_df = signalp_df[['prot_ID', 'sp_prob_logit', 'none_prob_logit']]
    signalp_df.to_csv(out_file, mode = 'a', sep='\t', index=False)

    return out_file

def process_signalp6_file(in_file, ayu_outdir, keep_files = False):
    signalp_proc_file = ayu_outdir + 'ayu.{}.signalp6.tsv'
    signalp_logit_file = ayu_outdir + 'ayu.{}.signalp6_logit.tsv'
    # Parse raw SignalP6 file
    signalp_proc_file = parse_signalp6_file(in_file, signalp_proc_file)
    # Transform files
...
def save_signalp6_file_in_ayu(sp_file, ayu_outdir):
    ayu_status_file, ayu_dict = ayu.preprocessing.get_ayu_status(ayu_outdir)
...
def process_ipc2_file(in_file, ayu_outdir):
    ipc2_file = ayu_outdir + 'ayu.{}.ipc2.tsv'.format(os.getpid())
